...
O pacote `magrittr` tem dois objetivos: diminuir o tempo de desenvolvimento e melhorar a legibilidade e a manutenção do código. 
O pacote `magrittr` oferece um conjunto de operadores que promovem semânticas que melhorara seu código que deixa mais claro a compreensão:
...
O `magrittr` também fornece três operadores de tubos relacionados. Eles não são tão comuns quanto, `%>%` mas se tornam úteis em casos especiais.
